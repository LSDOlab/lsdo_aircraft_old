...
class WaveDragCoeffComp(ArrayExplicitComponent):
    def array_setup(self):
        ...
        self.array_add_input('critical_mach')
        self.array_add_output('wave_drag_coeff')

        self.array_declare_partials('wave_drag_coeff', 'mach_number')
        self.array_declare_partials('wave_drag_coeff', 'critical_mach')

    def compute(self, inputs, outputs):
        d_mach = inputs['mach_number'] - inputs['critical_mach']
        mask = (d_mach > 0).astype(int)
        d_mach *= mask
        outputs['wave_drag_coeff'] = (20. * d_mach**4)

    def compute_partials(self, inputs, partials):
        d_mach = (inputs['mach_number'] - inputs['critical_mach']).flatten()
        mask = (d_mach > 0).astype(int)
        d_mach *= mask
        partials['wave_drag_coeff', 'mach_number'] = 80. * d_mach**3
        partials['wave_drag_coeff', 'critical_mach'] = -80. * d_mach**3
...
if __name__ == '__main__':
    ...
    comp.add_output('critical_mach', 10 * np.random.random(shape))
